...
As you can see with this graph, diabetes does cause a additional risk for mortality.  However, increasing prevalence of all diabetes nor type 1 increases this risk.  Increasing the prevalence of type 2 diabetes does increase the risk for death.

```{r}
source("../03 Visualizations/Visualization2.R", echo = TRUE)
```

As you can see with these graphs, diabetes does increase the risk for MI and stroke, but increasing prevalence of diabetes (all types) does not correlate with and increased risk of stroke nor MIs. 
